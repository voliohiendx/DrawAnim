...
class QueueLinearFloodFiller(val context: Context) {

    lateinit var image: Bitmap
    lateinit var imageInput: Bitmap
    var tolerance: IntArray = intArrayOf(0, 0, 0, 0)
    private var width: Int = 0
    private var height: Int = 0
    var fillColor: Int = 0
    private var startColor: IntArray = intArrayOf(0, 0, 0, 0)
    private lateinit var pixelsChecked: BooleanArray
    private lateinit var ranges: Queue<FloodFillRange>
    private lateinit var pixels: IntArray


    fun setDataImage(img: Bitmap) {
        imageInput = img
        width = img.width
        height = img.height
    ...
    fun floodFill(x: Int, y: Int, newColor: Int, onSuccess: (Bitmap) -> Unit) {

        fillColor = newColor
        setTargetColor(imageInput.getPixel(x, y))
        ...
        if (startColor[0] == 0) {
            ...
            startColor[3] = (startPixel shr 24) and 0xff
        }
    ...
    private fun checkPixel(px: Int): Boolean {
        ...
        return (alpha >= (startColor[3] - tolerance[3]) && alpha <= (startColor[3] + tolerance[3]) && red >= (startColor[0] - tolerance[0]) && red <= (startColor[0] + tolerance[0]) && green >= (startColor[1] - tolerance[1]) && green <= (startColor[1] + tolerance[1]) && blue >= (startColor[2] - tolerance[2]) && blue <= (startColor[2] + tolerance[2]))
    }
